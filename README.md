...
#### context.globalAlpha = 0到1之间的一个数字 包括0和1

这个属性用来指定绘制的图像的透明度

#### context.globalCompostionOperation 

这个属性用来表示绘制的图形和先绘制的图形是怎么结合的 

* source-over 后绘制的图形位于先绘制的图像上方

* source-in 后绘制的图像和先绘制的图像重叠的部分可见 其他两部分完全透明
...
跨文档信息传递简称XDM 他的核心就是postMessage方法  这个方法的目的就是为了向另一个地方传递数据  这里的另一个地方指的就是包含在当前页面的iframe元素或者由当前页面弹出的窗口 

**postMessage方法接受两个参数 第一个字符串表示传递的信息内容 如果想传入结构化的数据可以JSON.stringify 第二参数表示框架中的文档必须来源于什么域 如果来源匹配则发送消息。如果不匹配postMesage什么也不做 第二参数也可以是\* 来表示匹配任何域 不过不建议这样用**
